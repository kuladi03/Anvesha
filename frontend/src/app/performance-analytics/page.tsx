...
export default function PerformanceAnalytics() {
	...
	const [overallRisk, setOverallRisk] = useState<Risk>({
		value: 0,
		label: "Loading...",
	});
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/performance/${studentId}`
			);
			...
			const courseProgressArr = Object.entries(courseMap).map(
				([courseTitle, totalMinutes]) => ({
					courseTitle,
					totalMinutes,
				})
			);

			// const dailyProgressArr = Object.entries(dateMap)
			// 	.map(([date, duration]) => ({
			...
			// Fill missing dates with zero activity for daily progress
			const rawDates = Object.keys(dateMap).sort(
				(a, b) => new Date(a).getTime() - new Date(b).getTime()
			);

			let filledDates: DailyProgress[] = [];
			if (rawDates.length) {
				const startDate = new Date(rawDates[0]);
				const endDate = new Date(rawDates[rawDates.length - 1]);
			...
			setDailyProgressData(filledDates);
			...
			if (filledDates.length) {
				const uniqueDays = new Set(
					filledDates.filter((d) => d.duration > 0).map((d) => d.date)
				);
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
					<Card className="col-span-1 md:col-span-2">
						<CardHeader>
							...
							<CardDescription>
								Total minutes spent on each course
							</CardDescription>
						</CardHeader>
						<CardContent>
              ...
              <ResponsiveContainer width="100%" height={300}>
				...
				<Card className="mb-6">
					<CardHeader>
						...
						<CardDescription>
							Calculated from performance and activity
						</CardDescription>
					</CardHeader>
					<CardContent>
